...
function validaCelular(){
    ...
    console.log(regex.test(celular.value));
...
function validaEmail(){
    const regex = /\w+@\w+\.\w/; //o W é considerado como um seletor global, abrangendo letras maiúsculas e minúsculas, caracteres especiais e números.
...
function validaSenha(){
    const regex = /^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[!@#$%&*]).{6,15}$/; //O código ?=.* é usado para fazer uma busca no grupo correspondente de forma geral, sem priorizar se a string começa com aquele caractere ou não.
